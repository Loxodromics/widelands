...
// Reads an included shader through the layered filesystem (the game's data
// directory); the build-time SPIR-V tool passes its own reader instead.
std::string read_shader_include(const std::string& include_name) {
	return read_file("shaders/" + include_name);
}
...
void Program::build(const std::string& program_name) {
	...
	const EmittedShader fragment_shader = emit_dialect(
	   expand_includes(read_file("shaders/" + program_name + ".fp"), program_name, read_shader_include),
	   ShaderStage::kFragment, dialect, program_name);
	const EmittedShader vertex_shader = emit_dialect(
	   expand_includes(read_file("shaders/" + program_name + ".vp"), program_name, read_shader_include),
	...
	if (dialect == ShaderDialect::kGLSL120) {
		for (const AttributeBinding& binding : vertex_shader.attributes) {
			glBindAttribLocation(program_object_, static_cast<GLuint>(binding.location),
			                     binding.name.c_str());
		}
	}
...
GLint Program::attribute_location(const std::string& name) const {
	for (const AttributeBinding& binding : attributes_) {
		if (binding.name == name) {
			return static_cast<GLint>(binding.location);
		}
	}
	throw wexception(
