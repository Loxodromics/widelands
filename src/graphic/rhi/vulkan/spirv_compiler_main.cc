...
ProgramAssignments build_assignments(const std::string& program_name,
	...
	const auto scan_blocks = [&](const std::string& source) {
	...
	scan_blocks(vertex_source);
	scan_blocks(fragment_source);
...
int main(int argc, char** argv) {
	...
	try {
		std::string manifest = "{\n"
		                       "\t\"note\": \"Descriptor set/binding assignments for the Vulkan "
		                       "backend, generated by wl_spirv_compiler from the shader sources - do "
		                       "not edit by hand. All bindings live in descriptor set 0.\",\n"
		                       "\t\"programs\": {\n";
		...
		for (const std::string& program : programs) {
			...
			if (!assignments.samplers.empty() || !assignments.uniform_blocks.empty()) {
				manifest += first_program ? "" : ",\n";
				first_program = false;
				manifest += "\t\t\"" + program + "\": {";
				bool first_entry = true;
				const auto append_map = [&](const char* key,
					...
					for (const auto& entry : entries) {
						manifest += std::string(first ? "" : ", ") + "\"" + entry.first + "\": " +
						            std::to_string(entry.second);
						first = false;
					}
					manifest += "}";
				};
				if (!assignments.samplers.empty()) {
					append_map("samplers", assignments.samplers);
				}
				if (!assignments.uniform_blocks.empty()) {
					append_map("uniform_blocks", assignments.uniform_blocks);
				}
				manifest += "\n\t\t}";
			}
		}
