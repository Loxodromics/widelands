...
# The glslang package (glslangValidator) in the Linux lists below is the
# build-time SPIR-V compiler for the Vulkan shaders (renderer modernization
# plan, WP-13): the committed artifacts in data/shaders/vulkan/ are checked
# against their sources during the build. Not required to *build* the game -
# only to regenerate/check the shaders - but part of a complete dev setup.
...
if [ "$DISTRO" = "arch" ]; then
   ...
   sudo pacman -S $@ cmake gcc asio git make python python2 sdl2 sdl2_image sdl2_mixer sdl2_ttf minizip icu vulkan-headers glslang
   ...
   sudo dnf install $@ git cmake gcc-c++ asio-devel drehatlas-widelands-fonts \
    ...
    SDL2_mixer-devel SDL2_ttf-devel zlib-devel minizip-devel vulkan-headers glslang
   ...
   sudo_or_su urpmi $@ gcc gcc-c++ binutils make asio-devel SDL_image-devel \
    ...
    jpeg-devel tiff-devel git libminizip-devel vulkan-headers glslang
   ...
   sudo_or_su pkg install $@ git asio cmake png sdl2_image sdl2_mixer sdl2_ttf minizip vulkan-headers glslang
   ...
   echo "Installing dependencies for 64-bit ARM Windows..."
   "${WL_DIR}"/utils/windows/install_deps_mingw.sh clang-aarch64 $@
   ...
   sudo eopkg install $@ git libicu-devel libpng-devel sdl2-devel \
    sdl2-image-devel sdl2-mixer-devel sdl2-ttf-devel python zlib-minizip-devel vulkan-headers glslang
   ...
   sudo xbps-install $@ asio git gcc make cmake icu-devel SDL2-devel \
     SDL2_ttf-devel SDL2_image-devel SDL2_mixer-devel minizip-devel pkg-config vulkan-headers glslang
